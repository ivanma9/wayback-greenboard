...
function processHtml (html, baseUrl, allPages = null, currentPageIndex = 0, includeAssets = false, archiveId = null) {
  ...
  // ✅ Remove elements that might cause issues when viewing offline
  ...
  if (includeAssets) {
    ...
    // Only handle URLs that aren't already local assets
    $('img').each((i, img) => {
    ...
    $('link[rel="stylesheet"]').each((i, link) => {
      const href = $(link).attr('href')
      if (href && !href.startsWith('http') && !href.startsWith('./assets/')) {
        try {
          const absoluteUrl = new URL(href, baseUrl).href
    ...
    // Handle script tags that might have been missed
